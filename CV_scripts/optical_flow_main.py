...
def get_flow_command(prev_bgr, bgr, params):
    """ Calls the optical flow function to calcaulte the heading command.

    Args:
        prev_bgr (np.ndarray): Previous frame array
        bgr (np.ndarray): Curretn frame array
        params (dict): Paramter dict for the optical flow algorithm.
...
def time_analysis_subsampling(img_lst, params : dict, num_imgs : int = 5):
    ...
    for i,f in enumerate(subsampling_lst):
        ...
        factor_lst.append(1/f)

    # sort time in ascending order for better plots
    time_lst.sort()

    fig,ax = plt.subplots()
    # ax.set_title(f'Time Complexity Analysis')
    plt.loglog(factor_lst,time_lst,'+b')
    plt.loglog(factor_lst,time_lst,'-b')
    plt.xticks(np.unique(factor_lst),[])
    X_tick = [Fraction(item).limit_denominator() for item in factor_lst]
    plt.xticks(np.unique(factor_lst),X_tick)
