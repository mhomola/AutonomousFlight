...
static void calc_headign_commnad(struct image_t *img, struct opticflow_t *opticflow,
                                 struct opticflow_result_t *result);
                             ...
                             struct opticflow_result_t *result)
{
  ...
  // Expensive step, consider usign the white channle only (??)
  // !NOTE! -> arrow operator is the pointer to the member of a struct
  ...
  printf("Flow vectors shape: %d.\n",sizeof(struct flow_t))
  
  if (opticflow->track_back) {
    // TODO: Watch out!
    // !NOTE! This is actually False as a default -> not exectuing
...
float turn_angle  = 20.f;               // turn angle in ENU -- counter-clockwise positive [deg]
  ...
  switch (navigation_state) {
    ...
    case OBSTACLE_FOUND:
      // TODO Change behavior
       increase_nav_heading(turn_angle);
