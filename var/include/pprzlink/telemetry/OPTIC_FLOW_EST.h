 ...
 * @param _camera_id 
 */
static inline void pprzlink_msg_v2_send_OPTIC_FLOW_EST(struct pprzlink_msg * msg, float *_fps, uint16_t *_corner_cnt, uint16_t *_tracked_cnt, int16_t *_flow_x, int16_t *_flow_y, int16_t *_flow_der_x, int16_t *_flow_der_y, float *_yaw_command, uint8_t *_camera_id) {
#if PPRZLINK_ENABLE_FD
  long _FD = 0; /* can be an address, an index, a file descriptor, ... */
#endif
  const uint8_t size = msg->trans->size_of(msg, /* msg header overhead */4+4+2+2+2+4+4+1);
  if (msg->trans->check_available_space(msg, _FD_ADDR, size)) {
    msg->trans->count_bytes(msg, size);
    msg->trans->start_message(msg, _FD, /* msg header overhead */4+4+2+2+2+4+4+1);
    ...
    msg->trans->put_bytes(msg, _FD, DL_TYPE_FLOAT, DL_FORMAT_SCALAR, (void *) _yaw_command, 4);
...
#define DOWNLINK_SEND_OPTIC_FLOW_EST(_trans, _dev, fps, corner_cnt, tracked_cnt, flow_x, flow_y, flow_der_x, flow_der_y, yaw_command, camera_id) pprz_msg_send_OPTIC_FLOW_EST(&((_trans).trans_tx), &((_dev).device), AC_ID, fps, corner_cnt, tracked_cnt, flow_x, flow_y, flow_der_x, flow_der_y, yaw_command, camera_id)
/**
 ...
 * @param _camera_id 
 */
static inline void pprz_msg_send_OPTIC_FLOW_EST(struct transport_tx *trans, struct link_device *dev, uint8_t ac_id, float *_fps, uint16_t *_corner_cnt, uint16_t *_tracked_cnt, int16_t *_flow_x, int16_t *_flow_y, int16_t *_flow_der_x, int16_t *_flow_der_y,float *_yaw_command, uint8_t *_camera_id) {
    ...
    msg.receiver_id = 0;
    msg.component_id = 0;
    pprzlink_msg_v2_send_OPTIC_FLOW_EST(&msg,_fps,_corner_cnt,_tracked_cnt,_flow_x,
    _flow_y,_flow_der_x,_flow_der_y,_yaw_command,_camera_id);
}


#else // DOWNLINK

#define DOWNLINK_SEND_OPTIC_FLOW_EST(_trans, _dev, fps, corner_cnt, tracked_cnt, flow_x, flow_y, flow_der_x, flow_der_y, yaw_command, camera_id) {}
static inline void pprz_send_msg_OPTIC_FLOW_EST(struct transport_tx *trans __attribute__((unused)), struct link_device *dev __attribute__((unused)), uint8_t ac_id __attribute__((unused)), float *_fps __attribute__((unused)), uint16_t *_corner_cnt __attribute__((unused)), uint16_t *_tracked_cnt __attribute__((unused)), int16_t *_flow_x __attribute__((unused)), int16_t *_flow_y __attribute__((unused)), int16_t *_flow_der_x __attribute__((unused)), int16_t *_flow_der_y __attribute__((unused)), float *_yaw_command __attribute__((unused)), uint8_t *_camera_id __attribute__((unused))) {}
...
/** Getter for field yaw-command in message OPTIC_FLOW_EST
  *
  * @param _payload : a pointer to the OPTIC_FLOW_EST message
  * @return 
  */
static inline float pprzlink_get_DL_OPTIC_FLOW_EST_yaw_command(uint8_t * _payload __attribute__((unused)))
{
...
#define DL_OPTIC_FLOW_EST_yaw_command(_payload) pprzlink_get_DL_OPTIC_FLOW_EST_yaw_command(_payload)
